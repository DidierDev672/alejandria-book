...
<template>
  <BaseModal :is-open="isOpen" max-with-class="max-w-md" :expandable="false" @close="$emit('close')">
    <template #header>
      <h3 class="font-serif text-2xl font-bold tracking-wide text-white">Eliminar vínculo</h3>
      <p class="mt-1 text-sm text-amber-100/80">Esta acción se puede pensar con calma.</p>
    </template>
    <template #content>
      <p class="text-sm leading-relaxed text-stone-600">
        Vas a quitar el vínculo entre
        <span class="font-semibold text-stone-800">{{ userName }}</span>
        y
        <span class="font-semibold text-stone-800">{{ memberName }}</span>.
        No desaparece la persona ni su historia: solo esta unión. Si te equivocas, puedes crear una nueva cuando quieras.
      </p>
