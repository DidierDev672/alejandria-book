...
import { useAssignRolesMemberStore } from '../../../application/stores/useAssignRolesMemberStore'
import MemberListItem from '../molecules/MemberListItem.vue'

// ============================================================
// ORGANISM - Member selection modal
// ============================================================
...
const memberStore = useAssignRolesMemberStore()

// Fetch members when modal opens
watch(
  () => props.isOpen,
  (newVal) => {
    if (newVal && !memberStore.hasMembers) {
        ...
        <Transition enter-active-class="transition duration-300 ease-out"
          ...
          >
            ...
            <div class="bg-gradient-to-r from-amber-500 to-orange-600 px-6 py-4">
              <div class="flex items-center justify-between">
                <div class="flex items-center gap-3">
                  ...
                  <div>
                    <h2 class="text-lg font-serif font-bold text-white">Seleccionar Miembro</h2>
                    <p class="text-amber-100 text-xs">{{ memberStore.members.length }} miembros disponibles</p>
                  </div>
                </div>
                <button
                  type="button"
            ...
            <div class="px-6 py-4 max-h-96 overflow-y-auto">
              <!-- Loading -->
              <div v-if="memberStore.isLoading" class="flex flex-col items-center justify-center py-12">
                ...
                <p class="text-stone-500 text-sm">Cargando miembros...</p>
              </div>

              <!-- Error -->
              <div v-else-if="memberStore.error" class="text-center py-12">
                ...
                <p class="text-red-600 font-medium">{{ memberStore.error }}</p>
                <button
                  type="button"
              ...
              <!-- Empty state -->
              <div v-else-if="memberStore.filteredMembers.length === 0" class="text-center py-12">
                ...
                <p class="text-stone-600 font-medium">
                  {{ memberStore.searchQuery ? 'No se encontraron miembros' : 'No hay miembros disponibles' }}
                </p>
                <p class="text-stone-400 text-sm mt-1">
                  {{ memberStore.searchQuery ? 'Intenta con otros términos' : 'Crea un miembro primero' }}
                </p>
              </div>
            ...
            <div class="px-6 py-4 border-t border-stone-200 bg-stone-50">
              <div class="flex items-center justify-between">
                <p class="text-sm text-stone-500">
                  <span class="font-medium text-stone-700">{{ memberStore.filteredMembers.length }}</span>
                  resultado{{ memberStore.filteredMembers.length !== 1 ? 's' : '' }}
                </p>
                <button
                  type="button"
