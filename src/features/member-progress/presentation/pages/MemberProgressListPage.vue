...
<template>
  <div class="min-h-screen bg-gradient-to-br from-amber-50 via-orange-50/30 to-stone-100 px-6 py-8">
    ...
    <div class="max-w-7xl mx-auto space-y-6">
      ...
      <div v-motion :initial="{ opacity: 0, y: 8 }" :enter="{ opacity: 1, y: 0 }" :transition="{ delay: 0.1 }"
      ...
      <div v-if="loading" v-motion :initial="{ opacity: 0 }" :enter="{ opacity: 1 }"
      ...
      <div v-else-if="error" v-motion :initial="{ opacity: 0, scale: 0.98 }" :enter="{ opacity: 1, scale: 1 }"
        ...
        <button @click="fetchProgressMember"
          class="text-sm font-semibold text-amber-700 hover:text-amber-800 underline underline-offset-2">
          Reintentar
        </button>
      </div>

      <div v-else-if="list.length === 0" v-motion :initial="{ opacity: 0, y: 12 }" :enter="{ opacity: 1, y: 0 }"
      ...
      <div v-else v-motion :initial="{ opacity: 0, y: 16 }" :enter="{ opacity: 1, y: 0 }" :transition="{ delay: 0.15 }"
          ...
          <table class="w-full text-sm">
            <thead>
              <tr class="border-b border-amber-100 bg-amber-50/60">
                <th class="px-5 py-4 text-left text-xs font-semibold text-stone-400 uppercase tracking-widest">Miembro
                </th>
                <th class="px-5 py-4 text-left text-xs font-semibold text-stone-400 uppercase tracking-widest">Mes</th>
                <th class="px-5 py-4 text-left text-xs font-semibold text-stone-400 uppercase tracking-widest">Valor -
                  Kilos
                </th>
            ...
            <tbody class="divide-y divide-amber-50">
              <tr v-for="(progress, index) in list" :key="progress.id" v-motion :initial="{ opacity: 0, x: -8 }"
                ...
                <td class="px-5 py-4 text-right">
                  <div class="flex items-center justify-end gap-1.5">
                    <button @click.stop="openDetailModal(progress)"
                    ...
                    <button @click.stop="openDeleteModal(progress)"
    ...
    <BaseModal :is-open="detailModalOpen" max-with-class="max-w-lg" :expandable="false"
      ...
      <template #header>
        <div class="flex items-center gap-3">
          <div
            class="w-9 h-9 rounded-xl bg-white/20 border border-white/30 flex items-center justify-center shadow-sm shrink-0">
          ...
          <div>
            <h3 class="font-serif text-xl font-bold text-white leading-tight">Detalle del Progreso</h3>
            <p class="text-xs text-amber-100/60 mt-0.5">Registro de avance mensual</p>
          </div>
        </div>
      </template>
      ...
      <!-- ── Content ── -->
      <template #content>
        <div v-if="detailProgress" class="space-y-4">

          <!-- ══════════════════════════════
           § 1  KPI PRINCIPAL
           Valor del mes — lo más
           importante, al tope y grande.
           Patrón F-scan: la mirada va
           primero arriba a la izquierda.
      ══════════════════════════════ -->
          <div
            class="relative overflow-hidden rounded-2xl border border-amber-300/60 bg-gradient-to-br from-amber-50 to-orange-50/60">
            <div
              class="absolute inset-x-0 top-0 h-0.5 bg-gradient-to-r from-transparent via-amber-400/70 to-transparent" />

            <div class="flex items-center gap-5 px-5 py-5">

              <!-- Valor — jerarquía máxima en el modal -->
              <div class="shrink-0 text-center min-w-[72px]">
                <p class="font-serif font-black text-amber-700 text-4xl leading-none tracking-tight">
                  {{ detailProgress.recorded_value }}
                </p>
                <span class="text-[9px] font-black text-amber-500/80 uppercase tracking-[0.18em] mt-1 block">
                  valor registrado
                </span>
              </div>

              <!-- Separador vertical -->
              <div class="w-px self-stretch bg-amber-200/70 shrink-0" />

              <!-- Mes — jerarquía secundaria -->
              <div class="flex-1 min-w-0">
                <span class="text-[9px] font-black text-stone-400 uppercase tracking-[0.18em]">Período</span>
                <p class="font-serif font-bold text-stone-800 text-base mt-0.5 leading-tight">
                ...
                <div class="flex items-center gap-1.5 mt-2">
                  ...
                  <span class="text-[10px] text-stone-400 font-medium">Registro confirmado</span>
                </div>
              </div>
            </div>
          </div>
          ...
          <!-- ══════════════════════════════
           § 2  MIEMBRO
           Carga diferida con skeleton
           que ocupa el mismo espacio
           que el contenido real →
           evita layout shift (CLS).
      ══════════════════════════════ -->
          <section aria-label="Información del miembro">

            <!-- Encabezado de sección — patrón consistente -->
            <header class="flex items-center gap-2 mb-2.5">
              ...
              <h4 class="text-[10px] font-black text-stone-400 uppercase tracking-[0.15em]">Gladiador</h4>
              <div class="flex-1 h-px bg-gradient-to-r from-amber-200/70 to-transparent" />
            </header>

            <!-- Estado: cargando -->
            <div v-if="detailMemberLoading"
              class="bg-amber-50/40 rounded-2xl border border-amber-100 px-4 py-4 flex items-center gap-3">
              <div
                class="w-8 h-8 rounded-xl bg-amber-100 border border-amber-200 flex items-center justify-center shrink-0 animate-pulse">
                <div class="w-4 h-4 rounded-full bg-amber-200" />
              </div>
              <div class="flex-1 space-y-1.5">
                <!-- Skeleton lines — mismas proporciones que el texto real -->
                <div class="h-3.5 bg-amber-100 rounded-lg w-3/5 animate-pulse" />
                <div class="h-2.5 bg-amber-50 rounded-lg w-2/5 animate-pulse" />
              </div>
              <div class="w-4 h-4 border-2 border-amber-200 border-t-amber-500 rounded-full animate-spin shrink-0" />
            </div>

            <!-- Estado: datos cargados -->
            <div v-else-if="detailMember" class="bg-amber-50/40 rounded-2xl border border-amber-100 overflow-hidden">

              <!-- Nombre — fila primaria, máxima jerarquía dentro de la tarjeta -->
              <div class="px-4 pt-4 pb-3 border-b border-amber-100/80">
                <div class="flex items-center gap-2.5">
                  ...
                  <div
                    ...
                    <span class="text-sm font-black text-amber-700">
                      {{ detailMember.name_full?.charAt(0).toUpperCase() }}
                    </span>
                  </div>
                  <div class="flex-1 min-w-0">
                    <p class="font-serif font-bold text-stone-800 text-base leading-tight truncate">
                      {{ detailMember.name_full }}
                    </p>
                    <p class="text-[10px] text-stone-400 mt-0.5 capitalize">{{ detailMember.genre }}</p>
                  </div>
                </div>
              </div>

              <!-- Documento — fila secundaria, menor prominencia -->
              <div class="px-4 py-3 grid grid-cols-2 gap-3">
                <div>
                  <span class="text-[9px] font-black text-stone-400 uppercase tracking-[0.15em]">Tipo doc.</span>
                  <p class="text-sm font-semibold text-stone-700 mt-0.5">{{ detailMember.type_document }}</p>
                </div>
                <div>
                  <span class="text-[9px] font-black text-stone-400 uppercase tracking-[0.15em]">Número</span>
                  <p class="text-sm font-bold text-stone-700 mt-0.5 font-mono tracking-wide">
            ...
            <!-- Estado: sin datos -->
            <div v-else
              class="bg-amber-50/30 rounded-2xl border border-amber-100 px-4 py-3.5 flex items-center gap-2.5">
              <div
                class="w-7 h-7 rounded-lg bg-stone-100 border border-stone-200 flex items-center justify-center shrink-0">
          ...
          <!-- ══════════════════════════════
           § 3  OBSERVACIONES
           Solo aparece si hay notas.
           Texto corrido con ícono
           anclado → reduce tiempo de
           reconocimiento del tipo de
           contenido (principio de
           familiaridad de Norman).
      ══════════════════════════════ -->
          <section v-if="detailProgress.notes" aria-label="Observaciones">

            <header class="flex items-center gap-2 mb-2.5">
              <span
                class="flex h-5 w-5 items-center justify-center rounded-md bg-gradient-to-br from-stone-400 to-stone-500 shadow-sm shrink-0">
                <svg class="w-2.5 h-2.5 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    d="M7 8h10M7 12h4m1 8l-4-4H5a2 2 0 01-2-2V6a2 2 0 012-2h14a2 2 0 012 2v8a2 2 0 01-2 2h-3l-4 4z" />
                </svg>
              </span>
              <h4 class="text-[10px] font-black text-stone-400 uppercase tracking-[0.15em]">Observaciones</h4>
              <div class="flex-1 h-px bg-gradient-to-r from-stone-200/70 to-transparent" />
            </header>

            <div class="bg-stone-50/60 rounded-2xl border border-stone-200/60 px-4 py-3.5">
              <p class="text-sm text-stone-600 leading-relaxed">{{ detailProgress.notes }}</p>
            </div>
    ...
    <BaseModal :is-open="deleteModalOpen" max-with-class="max-w-sm" :expandable="false"
      @close="deleteModalOpen = false">
      <template #header>
        <h3 class="font-serif text-xl font-bold text-white">Confirmar Eliminación</h3>
      </template>
      <template #content>
        <p class="text-stone-600">¿Estás seguro de eliminar este registro de progreso? Esta acción no se puede deshacer.
        </p>
      </template>
      <template #footer>
